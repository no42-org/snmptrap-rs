...
- [ ] [Review][Decision] **`deny.toml` allowlist drift from D8** — `deny.toml:5-14` includes `Unicode-3.0`, but design D8 + `tasks.md §1.5` only authorize `MIT, Apache-2.0, MIT OR Apache-2.0, BSD-2-Clause, BSD-3-Clause, ISC, Zlib, Unicode-DFS-2016`. The `Unicode-3.0` SPDX is the modern replacement for `Unicode-DFS-2016` and is reachable via `unicode-ident`. Decide: (a) drop `Unicode-3.0` from `deny.toml`, identify the dep that requires it, and either swap or block; (b) amend D8 to add `Unicode-3.0` explicitly (group 4 spec edit) and keep `deny.toml` as-is; (c) defer until a Dependabot bump forces resolution.
- [ ] [Review][Decision] **Broaden `cargo-deny` to `check all`** — CI runs only `cargo-deny check licenses`. Despite `deny.toml` configuring `[advisories]` (yanked = warn) and `[bans]` (wildcards = deny), neither check is enforced. Switching to `command: check all` (or omitting the subcommand) enables advisories + bans + sources. Decide: (a) switch now, accept any new failures it surfaces; (b) keep licenses-only and document the gap as deferred; (c) run the broader checks on a parallel non-blocking job first to assess impact.
- [ ] [Review][Decision] **Release artifact provenance / signing** — `.github/workflows/release.yml` uploads binaries with same-runner-generated `*.sha256` and no signing. For a tool that does raw-IP source spoofing, supply-chain integrity is worth strengthening. Decide: (a) add `actions/attest-build-provenance` (cheapest — needs `id-token: write`); (b) add cosign/sigstore signing (more involved); (c) add GPG signing with a maintainer key (highest friction); (d) defer.
